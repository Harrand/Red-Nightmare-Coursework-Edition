...
Creature::Creature(BaseEngine* engine, const std::string& sprite_directory, unsigned int health, const Vector2i& position, unsigned int speed, unsigned int aggro_range, int activation_time, bool draggable): DisplayableObject(engine), Sprite(sprite_directory), max_health(health), health(health), speed(speed), aggro_range(aggro_range), activation_time(activation_time), fleeing(false), draggable(draggable), target(position), drag_offset(false, {0, 0})
{
